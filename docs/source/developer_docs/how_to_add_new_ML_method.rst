...
For a detailed documentation of how to implement each method, as well as some special cases, please see the docstrings of the :py:obj:`~immuneML.ml_methods.classifiers.MLMethod.MLMethod` class.


Adding a first example ML method
*********************************

Example: SillyClassifier
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

For this tutorial, we provide a :code:`SillyClassifier` (:download:`download here <./example_code/SillyClassifier.py>` or view below), in order to test adding a new MLMethod file to immuneML.
This method ignores the input dataset, and makes a random prediction per example.

.. collapse:: View SillyClassifier.py
...
#. Implement all abstract methods from the :py:obj:`~immuneML.ml_methods.MLMethod.MLMethod` class.
   See the class documentation for more detailed descriptions of the implementation of these methods.
   In the :code:`SillyClassifier` example code, these have already been implemented.
...
.. collapse:: View export_design_matrix.yaml
