...
# 2) The file with version info accessible by the immuneML library:
...
# Define the text for creating .py files for immuneML:
...
def generate_version_string(version):
    """Generate a string with the current immuneML version.

    Parameters
    ----------
    version : dict
        A dict containing the current immuneML version.

    Returns
    -------
    version_txt : string
        A string with the current immuneML version.
...
def get_version_info(version):
    """Return the version number for immuneML.
...
def write_version_py(version):
    """Create a file with the version info for immuneML.
    ...
    -------
    full_version : string
        The current full version for immuneML.
