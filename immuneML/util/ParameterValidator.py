...
class ParameterValidator:
    ...
    def assert_any_value_present(values: list, expected_values: list, location: str, parameter_name: str):
        assert any(exp_val in values for exp_val in
                   expected_values), f"{location}: expected at least one of values {expected_values} for " \
    ...
    @staticmethod
    def assert_all_type_and_value(values, parameter_type, location: str, parameter_name: str, min_inclusive=None,
                                  max_inclusive=None):
        for value in values:
            ParameterValidator.assert_type_and_value(value, parameter_type, location, parameter_name, min_inclusive,
                                                     max_inclusive)
    ...
    def assert_type_and_value(value, parameter_type, location: str, parameter_name: str,
        ...
        assert isinstance(value,
                          parameter_type), f"{base_mssg}It has to be of type {type_name}, but is now of type {type(value).__name__}."
        ...
        if exclusive:
            if len(keys) > len(valid_keys):
                raise AssertionError(
                    f"{location}: {str(list(set(keys) - set(valid_keys)))[1:-1]} are not valid parameters "
    ...
    @staticmethod
    def assert_valid_tabular_file(file_path, location: str, parameter_name: str, sep="\t",
        ...
        if expected_columns is not None:
            ...
            assert set(columns) == set(
                expected_columns), f"{location}: columns for {parameter_name} are not as expected.\n" \
    ...
    def assert_sequence_type(params, location: str = ""):
        ...
        if not isinstance(params['sequence_type'], SequenceType):
            assert params['sequence_type'].upper() in [st.name for st in SequenceType], \
                (f"{location}: {params['sequence_type']} is not a valid sequence type. "
                 f"Valid sequence types are: {[st.name for st in SequenceType]}.")

    @staticmethod
    def assert_region_type(params, location: str = ""):
        assert "region_type" in params, f"{location}: 'region_type' is missing: {params}."
        if not isinstance(params['region_type'], RegionType):
