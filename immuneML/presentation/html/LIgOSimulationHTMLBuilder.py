...
class LIgOSimulationHTMLBuilder:
    ...
    @staticmethod
    def build(state: LIgOSimulationState) -> str:
        """
    ...
    @staticmethod
    def make_html_map(state: LIgOSimulationState, base_path: Path) -> dict:
            ...
            "formats": [
                {
                    ...
                    "dataset_download_link": os.path.relpath(
                        path=Util.make_downloadable_zip(state.result_path, state.paths[state.dataset.name][format_name]),
                        start=base_path)
                } for format_name in state.formats
            ],
            "simulation_items": [Util.to_dict_recursive(implanting, base_path) for implanting in state.simulation.simulation_items]
        }
