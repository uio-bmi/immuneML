...
class ApplyGenModelHTMLBuilder:
    CSS_PATH = EnvironmentSettings.html_templates_path / "css/custom.css"

    @staticmethod
    def build(state: ApplyGenModelState) -> Path:
        base_path = PathBuilder.build(state.result_path / "../HTML_output/")
        html_map = ApplyGenModelHTMLBuilder.make_html_map(state, base_path)
        result_file = base_path / f"DatasetExport_{state.name}.html"

        TemplateParser.parse(template_path=EnvironmentSettings.html_templates_path / "ApplyGenModel.html",
                             template_map=html_map, result_path=result_file)

        return result_file

    @staticmethod
    def make_html_map(state: ApplyGenModelState, base_path: Path) -> dict:
        html_map = {
            "css_style": Util.get_css_content(ApplyGenModelHTMLBuilder.CSS_PATH),
            "name": state.name,
