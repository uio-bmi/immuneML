...
class WeightHelper:
    ...
    @staticmethod
    def compute_column_contributions(column, alphabet=None, pseudocount_value=1):
    ...
    @staticmethod
    def compute_positional_aa_contributions(np_sequences, pseudocount_value=1):
        return {idx: WeightHelper.compute_column_contributions(np_sequences[:, idx],
                                                               pseudocount_value=pseudocount_value)
                for idx in range(len(np_sequences[0]))}

    @staticmethod
    def compute_sequence_weight(sequence, positional_weights, lower_limit=0, upper_limit=np.inf, alphabet=None):
        alphabet = EnvironmentSettings.get_sequence_alphabet() if alphabet is None else alphabet
