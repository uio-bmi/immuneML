...
from immuneML.ml_methods.classifiers.fixed_BrokenLogisticRegression import BrokenLogisticRegression
from immuneML.util.PathBuilder import PathBuilder


class TestBrokenLogisticRegression(TestCase):
    ...
    def test_store(self):
        ...
        lr.store(path)
        self.assertTrue(os.path.isfile(path / "broken_logistic_regression.pickle"))

        with open(path / "broken_logistic_regression.pickle", "rb") as file:
    ...
    def test_load(self):
        ...
        with open(path / "broken_logistic_regression.pickle", "wb") as file:
            pickle.dump(lr.model, file)

        lr2 = BrokenLogisticRegression()
        lr2.load(path)
