...
class TestManualSplitter(TestCase):
    def test__split_repertoire_dataset(self):
        ...
        train_metadata = pd.DataFrame({"donor": ["rep_1", "rep_2", "rep_4", "rep_5", "rep_9", "rep_7"]})
        train_metadata.to_csv(path + "train.csv")

        test_metadata = pd.DataFrame({"donor": ["rep_0", "rep_3", "rep_6", "rep_8"]})
        ...
        self.assertTrue(all(donor in ["rep_1", "rep_2", "rep_4", "rep_5", "rep_9", "rep_7"]
                            for donor in train_datasets[0].get_metadata(["donor"])["donor"]))
        self.assertTrue(all(donor in ["rep_0", "rep_3", "rep_6", "rep_8"]
                            for donor in test_datasets[0].get_metadata(["donor"])["donor"]))
