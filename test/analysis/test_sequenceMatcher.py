...
class TestSequenceMatcher(TestCase):
    ...
    def test_match(self):
        ...
        repertoire = Repertoire.build_from_sequence_objects(sequence_objects=[
            ReceptorSequence(amino_acid_sequence="AAAAAA", metadata=SequenceMetadata(chain="A", v_call="V1", j_call="J2"), identifier="3"),
            ReceptorSequence(amino_acid_sequence="CCCCCC", metadata=SequenceMetadata(chain="A", v_call="V1", j_call="J2"), identifier="4"),
            ReceptorSequence(amino_acid_sequence="AAAACC", metadata=SequenceMetadata(chain="A", v_call="V1", j_call="J2"), identifier="5"),
            ReceptorSequence(amino_acid_sequence="TADQVF", metadata=SequenceMetadata(chain="A", v_call="V1", j_call="J3"), identifier="6")],
            metadata={"CD": True}, path=path)

        dataset = RepertoireDataset(repertoires=[repertoire])
        sequences = [ReceptorSequence("AAAACA", metadata=SequenceMetadata(chain="A", v_call="V1", j_call="J2"), identifier="1"),
                     ReceptorSequence("TADQV", metadata=SequenceMetadata(chain="A", v_call="V1", j_call="J3"), identifier="2")]
