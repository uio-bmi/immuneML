...
def test_implant_in_repertoire():
    ...
    assert len(repertoire.sequences) == len(new_repertoire.sequences)
    assert 1 == len([seq for seq in new_repertoire.sequences if seq.sequence_aa == "AAAA" and seq.metadata.v_call == "v1"])
    assert 2 == len([seq for seq in new_repertoire.sequences if seq.sequence_aa == "CCCC" and seq.metadata.v_call != "v1"])
    assert new_repertoire.get_region_type() == RegionType.IMGT_JUNCTION
