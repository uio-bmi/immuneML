...
class TestAtchleyKmerEncoder(TestCase):
    ...
    def test_encode(self):
        ...
        encoded_dataset = encoder.encode(dataset, EncoderParams(path / "result",
                                                                LabelConfiguration(labels=[Label("l1", [True, False])]),
                                                                region_type=RegionType.IMGT_CDR3))

        self.assertEqual(3, len(set(list(itertools.chain.from_iterable([rep.data.cdr3_aa.tolist() for rep in dataset.repertoires])))))
