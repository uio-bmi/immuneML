...
class ComparisonData:
    ...
    def __init__(self, repertoire_ids: list, matching_columns, item_columns: list, path: str = None, batch_size: int = 10000):
    ...
    def filter_existing_items(self, items: pd.DataFrame, repertoire_id: str) -> pd.DataFrame:
        ...
        for batch_index, batch in enumerate(self.get_batches()):
            new_items = self._remove_existing_items(new_items, batch, self.batch_paths[batch_index], repertoire_id)
        return new_items

    def _remove_existing_items(self, new_items: pd.DataFrame, batch: pd.DataFrame, batch_path: str, repertoire_id: str) -> pd.DataFrame:
        ...
        for index, item in new_items.iterrows():
            ...
            if occurrences == 0:
                ...
                batch["rep_{}".format(repertoire_id)] += matches.astype(int)
