...
class DatasetMerger(Step):
    ...
    def perform_step(input_params: dict = None):
        ...
        sample_parameter_names = DatasetMerger._extract_sample_parameter_names(datasets)
        new_sample_names = DatasetMerger._build_new_sample_names(sample_parameter_names, input_params)
        dataset = DatasetMerger._build_dataset(input_params, list(new_sample_names))
        return dataset

    @staticmethod
    def _build_dataset(input_params: dict, new_sample_names: list):
        dataset = Dataset()
        file_paths = []
        ...
        for ds in input_params["datasets"]:
            file_paths.extend(DatasetMerger._process_dataset(ds, input_params, new_sample_names))
    ...
    @staticmethod
    def _process_dataset(dataset: Dataset, input_params: dict, new_sample_names: list):
        ...
        for repertoire in dataset.get_data(batch_size):
            file_path = DatasetMerger._process_repertoire(repertoire, input_params, new_sample_names)
            file_paths.append(file_path)
        return file_paths

    @staticmethod
    def _process_repertoire(repertoire: Repertoire, input_params: dict, new_sample_names: list):
        """
    ...
    @staticmethod
    def _build_new_sample_names(all_sample_names: set, input_params: dict):
    ...
    @staticmethod
    def _extract_sample_parameter_names(datasets: list):
