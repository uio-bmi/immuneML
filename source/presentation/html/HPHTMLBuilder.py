...
class HPHTMLBuilder:
    ...
    def build(state: HPOptimizationState = None) -> str:
        ...
        html_map = HPHTMLBuilder.make_main_html_map(state)
        ...
        for index, item in enumerate(HPHTMLBuilder.make_assessment_indices_list(state)):
        ...
        for label in state.label_configuration.get_labels_by_name():
            for assessment_index in range(state.assessment.split_count):
                TemplateParser.parse(template_path=f"{EnvironmentSettings.html_templates_path}SelectionDetails.html",
                                     template_map=HPHTMLBuilder.make_selection(state, assessment_index, label),
    ...
    def make_selection_split_path(assessment_index: int, state: HPOptimizationState, label: str):
        return f"{state.assessment_states[assessment_index].path}selection_{state.selection.split_strategy.name.lower()}/" \
               f"{label}SelectionDetails.html"

    @staticmethod
    def make_selection(state: HPOptimizationState, assessment_index: int, label: str):
    ...
    @staticmethod
    def make_assessment_indices_list(state: HPOptimizationState):
        return [{"css_style": Util.get_css_content(HPHTMLBuilder.CSS_PATH),
                 ...
                 "labels": [{
                     ...
                     "selection_path": HPHTMLBuilder.make_selection_split_path(i, state, label)
                 } for label in state.label_configuration.get_labels_by_name()],
                 }
    ...
    def make_hp_per_label(state: HPOptimizationState):
        return [{"label": label, "assessment_results":
            [{"index": assessment_state.split_index + 1,
              ...
              "split_details_path": HPHTMLBuilder.make_assessment_split_path(assessment_state.split_index, state)}
             for i, assessment_state in enumerate(state.assessment_states)]} for label in
                state.label_configuration.get_labels_by_name()]

    @staticmethod
    def make_main_html_map(state: HPOptimizationState) -> dict:
        html_map = {
            "css_style": Util.get_css_content(HPHTMLBuilder.CSS_PATH),
            ...
            "dataset_reports": Util.to_dict_recursive(state.data_report_results) if state.data_report_results else None,
            "show_dataset_reports": bool(state.data_report_results),
            "show_hp_reports": bool(state.hp_report_results),
            'hp_reports': Util.to_dict_recursive(state.hp_report_results) if state.hp_report_results else None,
