...
class SVM(MLMethod):
    ...
    def _fit_for_label(self, X: Iterable, y: np.ndarray, label: str, cores: int):
        self._models[label] = SGDClassifier(loss="hinge", n_jobs=cores, tol=1e-3)  # hinge loss + SGD classifier -> SVM
    ...
    def predict(self, X: Iterable, label_names: list = None):
        ...
        assert all([isinstance(self._models[label],
                               SGDClassifier) or isinstance(self._models[label], RandomizedSearchCV) for label in
    ...
    def _fit_for_label_by_cv(self, X: Iterable, y: np.ndarray, label: str, cores: int, number_of_splits: int = 5):
        self._models[label] = RandomizedSearchCV(SGDClassifier(loss="hinge", tol=1e-3),
